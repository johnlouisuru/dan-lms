...
<!-- Total Modules -->
<?php
  // Query to count total modules uploaded
  $stmt = $pdo->query("SELECT COUNT(id) AS total_modules FROM topic_modules");
  $row  = $stmt->fetch(PDO::FETCH_ASSOC);

  $total_modules = $row ? $row['total_modules'] : 0;
?>
  ...
  <a href="../e-module">
    <div class="card">
      <div class="card-header p-2 ps-3">
        <div class="d-flex justify-content-between">
          <div>
            <p class="text-sm mb-0 text-capitalize">Total Modules</p>
            <h4 class="mb-0"><?= htmlspecialchars($total_modules) ?></h4>
          </div>
          <div class="icon icon-md icon-shape bg-gradient-dark shadow-dark shadow text-center border-radius-lg">
...
<!-- Total Sections (Commented out for now) -->
<?php
