            ...
            <tbody>
                ...
                <?php foreach ($students as $fetched): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php 
                            $percentage_holder = get_requirement_percentage($pdo, $fetched['id']);
                                if($percentage_holder == 100){ ?>
                                    <div class="progress">
                                        <div class="progress-bar bg-success" role="progressbar" style="width: <?=$percentage_holder?>%;" aria-valuenow="<?=$percentage_holder?>" aria-valuemin="0" aria-valuemax="100"><?=round($percentage_holder, 2)?>%</div>
                                    </div> 
                                <?php 
                                }
                                else { ?>
                                    <div class="progress">
                                <div class="progress-bar bg-danger" role="progressbar" style="width: <?=$percentage_holder?>%;" aria-valuenow="<?=$percentage_holder?>" aria-valuemin="0" aria-valuemax="100"><?=round($percentage_holder, 2)?>%</div>
                            </div>
                                <?php 
                                }
                            ?>
