            ...
            <?php
                ...
                $query_newsfeed = "
                        SELECT 
                            ...
                            nf.feed_title,
                            nf.feed_content,
                            nf.file_attached_path,
                            nf.teacher_id,
                            nf.student_id,
                            nf.created_at AS time_display,
                        ...
                        FROM newsfeed nf
                        LEFT JOIN teachers t ON nf.teacher_id = t.id
                        LEFT JOIN students s ON nf.student_id = s.id
            ...
            ?>
                <?php if ($newsfeed && $newsfeed->rowCount() > 0): ?>
                    <?php foreach ($newsfeed as $feed): 
                        $isTeacherPost = !empty($feed['teacher_id']);
                        $isStudentPost = !empty($feed['student_id']);

                        // Author name
                        $authorName = $isTeacherPost
                            ? $feed['teacher_firstname'] . ' ' . $feed['teacher_lastname']
                            : $feed['student_firstname'] . ' ' . $feed['student_lastname'];

                        // Avatar initials
                        $avatar = strtoupper(
                            ($isTeacherPost ? $feed['teacher_firstname'][0] : $feed['student_firstname'][0]) .
                            ($isTeacherPost ? $feed['teacher_lastname'][0]  : $feed['student_lastname'][0])
                        );

                        // Attachment path handling
                        $filePath = null;
                        if (!empty($feed['file_attached_path'])) {
                            $filePath = $isStudentPost
                                ? '../final/' . $feed['file_attached_path']
                                : $feed['file_attached_path'];
                            }
                        ?>
                        <div class="feed-card position-relative">

                            <?php if ($isTeacherPost && $feed['teacher_id'] == $_SESSION['user_id']): ?>
                                <div class="dropdown feed-menu">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <button class="dropdown-item edit-feed-btn"
                                        ...
                                        <li>
                                            <button class="dropdown-item text-danger delete-feed-btn"
                                                data-id="<?= $feed['feed_id'] ?>">
